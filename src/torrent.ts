...
import { ExtendedInstance, Room, TorrentRoomConfig } from "./types";
import {
  encodeBytes,
  entries,
  events,
...
export const joinRoom = initGuard(
  occupiedRooms,
  (config: TorrentRoomConfig, ns: string | number) =>
    new Promise<Room>(async (resolve, reject) => {
      ...
      if (!trackerUrls.length) {
        throw reject(() => mkErr("trackerUrls is empty"));
      }
        ...
        if (forced || !sockets[url]) {
          ...
          sockets[url] = new Promise((res) => {
            ...
            socket.onmessage = (err) =>
              values(socketListeners[url]).forEach((f) => f(socket, err));
          });
        } else {
